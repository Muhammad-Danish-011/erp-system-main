...
const settingsNav = { name: "Settings", href: "/settings", icon: SettingsIcon };
  const toggleMenu = (menuName: string) => {
    setOpenMenus((prev) =>
      prev.includes(menuName)
        ? prev.filter((m) => m !== menuName)
  ...
  return (
    <>
      {/* Mobile Menu Button */}
      <div className="lg:hidden fixed top-[13px] left-2 z-50">
        <button
      ...
      <div
        className={cn(
          "fixed lg:static top-0 left-0 z-50 flex flex-col bg-gray-900 transition-all duration-300 overfoow-y-auto h-screen",
          isCollapsed ? "w-16" : "w-64",
          // mobile behavior
          ...
          {!isCollapsed && (
            <>
              <img
                ...
                className="h-auto w-auto max-h-14 max-w-14 object-contain"
              />
              <h1 className="text-xl font-bold text-white hidden sm:block whitespace-nowrap">
                ERP System
              </h1>
            </>
          )}
